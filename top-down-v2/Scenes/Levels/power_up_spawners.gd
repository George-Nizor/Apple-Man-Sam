...
func _filter_unused_spawners(spawner):
	return not used_spawners.has(spawner)
...
func spawn_powerup(spawner_list):
	...
	var valid_spawners = spawners.duplicate().filter(_filter_unused_spawners)
	if len(used_spawners) == len(spawner_list):
		valid_spawners = spawner_list
		used_spawners.clear()
	# Select from valid spawners
	selected_spawner = valid_spawners[rng.randi() % valid_spawners.size()]
	used_spawners.append(selected_spawner)
